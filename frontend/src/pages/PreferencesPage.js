...
export default function PreferencesPage() {
  ...
  useEffect(() => {
    try {
      const savedRaw = localStorage.getItem(SAVED_PREFERENCES_KEY);
      ...
      const latestSaved = savedList[0];
      const latestPreferences = latestSaved?.userPreferences;

      if (!latestPreferences) {
        return;
      }

      setSavedPreference(latestSaved);
      setFormData((prev) => ({
        ...prev,
        scentProfile: latestPreferences.scentProfile || "",
        priceRange: latestPreferences.priceRange || "",
        gender: latestPreferences.gender || "",
        occasion: latestPreferences.occasion || "",
      }));
    } catch (loadError) {
      console.error("Failed to load saved preferences:", loadError);
    }
  }, []);
      ...
      <div className="preferences-root">
        <div className="preferences-container">
          ...
          <div className="preferences-content">
            ...
            {savedPreference?.userPreferences && (
              <div className="saved-preference-card">
                <p className="saved-preference-title">
                  Latest Saved Preference
                </p>
                <div className="saved-preference-grid">
                  <p>
                    <strong>Scent:</strong>{" "}
                    {formatValue(savedPreference.userPreferences.scentProfile)}
                  </p>
                  <p>
                    <strong>Price:</strong>{" "}
                    {formatValue(
                      savedPreference.userPreferences.priceRange.replace(
                        /\s+/g,
                        "-",
                      ),
                    )}
                  ...
                  <p>
                    ...
                    {formatValue(savedPreference.userPreferences.gender)}
                  </p>
                  <p>
                    <strong>Occasion:</strong>{" "}
                    {formatValue(savedPreference.userPreferences.occasion)}
                  </p>
                </div>
              </div>
            )}
